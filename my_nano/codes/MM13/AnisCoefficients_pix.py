...
def dip_createSignalResponse(pphi, ptheta, dipphi, diptheta):
    ...
    @param pphi:        Phi of the pulsars
    @param ptheta:      Theta of the pulsars
    @param dipphi:      Phi of Dipole location (missing body)
    @param diptheta:    Theta of Dipole location (missing body)
...
def dip_createSignalResponse_pol(pphi, ptheta, dipphi, diptheta, ksi=0.0, norm=True):
    ...
    @param pphi:        Phi of the pulsars
    @param ptheta:      Theta of the pulsars
    @param dipphi:      Phi of Dip location (missing body)
    @param diptheta:    Theta of Dip location (missing body)
...
def CorrBasis(psr_locs, lmax, nside=32, ephem=False):
    """
    ...
    @param ephem:       Whether we do ephemeris missing bodies, or
                        GR-polarisations (F+ and Fx).
                        False = GR (default)
                        True = Ephemeris
    ...
    if ephem:
        F_e = dip_signalResponse_fast(ptheta, pphi, gwtheta, gwphi)
    else:
